...
export default function BottomTabNav() {
  ...
  return (
    <nav
      className="cf-safe-bottom fixed inset-x-0 bottom-0 z-40 border-t border-[var(--cf-border)] bg-[var(--cf-surface)] lg:hidden"
      aria-label="Primary"
    >
          ...
          return (
            <li key={item.path} className="flex min-w-0 flex-1">
              <Link
                ...
                className={[
                  'flex min-h-11 w-full min-w-0 flex-col items-center justify-center gap-0.5 px-1',
                  'focus-visible:outline-2 focus-visible:outline-[var(--cf-brand)] focus-visible:outline-offset-[-2px]',
                  active ? 'text-[var(--cf-brand)]' : 'text-[var(--cf-text-tertiary)]',
                ].join(' ')}
              >
